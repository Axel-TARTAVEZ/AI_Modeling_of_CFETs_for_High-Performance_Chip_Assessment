...
def progress_monitor(q, total_epochs):
    ...
    while True:
        ...
        # Les espaces à la fin écrasent les anciens caractères si la ligne rétrécit
...
if __name__ == '__main__':
    ...
    joblib.dump(scaler_X, os.path.join(model_dir, '2DFET_AI_scaler_x.pkl'))
    joblib.dump(scaler_y, os.path.join(model_dir, '2DFET_AI_scaler_y.pkl'))

    model_champion = CFET_AI(input_dim, output_dim)
    model_champion.load_state_dict(global_best_weights)
    torch.save(model_champion.state_dict(), os.path.join(model_dir, "2DFET_AI_Model.pth"))

    model_champion.eval()
    dummy_input = torch.randn(1, input_dim)
    torch.onnx.export(model_champion, dummy_input, os.path.join(model_dir, "2DFET_AI_Model.onnx"), 
