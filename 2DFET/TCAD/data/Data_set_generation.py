...
df_valid_sims = df.dropna()
df_valid_sims = df_valid_sims[(df_valid_sims != -999.0).all(axis=1)]

beta_ratio = df_valid_sims['Ion_N'] / df_valid_sims['Ion_P']

invalid_mask = (beta_ratio < 0.35) | (beta_ratio > 2)
df_aberrant = df_valid_sims[invalid_mask]

nb_999 = len(df_999)
nb_aberrant = len(df_aberrant)
total_crashes_aberrations = nb_999 + nb_aberrant

df_clean = df_valid_sims[~invalid_mask].reset_index(drop=True)
...
print(f"-> TCAD Crashes / Aberrations    : {total_crashes_aberrations} ({nb_aberrant} Beta Ratios Aberrations)")
print("--------------------------------------------------")
